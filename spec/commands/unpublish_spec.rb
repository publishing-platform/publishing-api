...
RSpec.describe Commands::Unpublish do
  before do
  ...
  describe "call" do
    ...
    context "when unpublishing is invalid" do
      ...
      it "raises an error when invalid unpublishing type" do
        msg = /is not a valid unpublishing type/
        expect { described_class.call(payload.merge(type: "invalid_type")) }
          .to raise_error(CommandError, msg) do |error|
            expect(error.code).to eq(422)
          end
      end
    ...
    context "when the document is published" do
      ...
      it "sends an unpublishing downstream" do
        expect(DownstreamDraftJob).to receive(:perform_async)
          .with(
            a_hash_including("content_id" => content_id, "source_command" => "unpublish"),
          )
    ...
    context "when there is a draft and published with differing links" do
      ...
      it "excludes orphaned content ids downstream draft as they were handled in put content" do
        expect(DownstreamDraftJob).to receive(:perform_async)
          .with(hash_excluding("orphaned_content_ids"))
      end
    end
