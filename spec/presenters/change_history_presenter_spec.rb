...
RSpec.describe Presenters::ChangeHistoryPresenter do
  ...
  describe "#change_history" do
    ...
    context "details hash doesn't include content_history" do
      before do
        create(:change_note, edition: live_edition, note: "0", public_timestamp: 1.day.ago)
        create(:change_note, edition:, note: "1", public_timestamp: Time.zone.now.utc)
      end
    ...
    it "orders change notes by public_timestamp (ascending)" do
      create(:change_note, edition: live_edition, note: "0", public_timestamp: Time.zone.now.utc)
      create(:change_note, edition:, note: "1", public_timestamp: 1.day.ago)

      expect(subject.map { |item| item[:note] }).to eq %w[1 0]
    end

    it "omits change notes that don't have a public timestamp" do
      create(:change_note, edition: live_edition, note: "with-timestamp", public_timestamp: Time.zone.now.utc)
